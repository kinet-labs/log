...
// We start with github.com/kinet-labs/log and zap internals; callers can add more at runtime.
var internalPkgList atomic.Value // stores []string

func init() {
	internalPkgList.Store([]string{
		"github.com/kinet-labs/log",
		"go.uber.org/zap",
		"go.uber.org/zap/zapcore",
	})
}
...
func isInternalFrame(f runtime.Frame) bool {
	...
	// Check for github.com/kinet-labs/log in any form (versioned or not)
	if strings.Contains(f.File, "github.com/kinet-labs/log") {
		return true
	}
